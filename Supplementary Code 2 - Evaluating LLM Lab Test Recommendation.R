...
recommended_tests2 <- read.csv("recommended_tests_manual_check_names CHECKED.csv") %>% select(-uzl_test_name, -category, -menu)
...
rm(expanded_tests)
recommended_tests_clean4 <- read.csv("recommended_tests_manual_check_multiple_codes CHECKED.csv") %>% 
...
recommended_tests_clean5 <- read.csv("recommended_tests_manual_check_remaining CHECKED.csv") %>% 
  select(recommended_test_name, test_code, n, correctness, reviewed_test_code)
...
first_delphi_round <- read.csv("First_Delphi_expert_tests.csv")
...
case_complexity <- read.csv("case_complexity.csv")
...
panel_codes <- read.csv("panel codes.csv")
